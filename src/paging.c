...
struct PageDirectory* paging_create_new_page_directory(void) {
    ...
    struct PageDirectory* new_page_dir = &page_directory_list[entry];
    memset(new_page_dir, 0, sizeof(struct PageDirectory));

    new_page_dir->table[0] = (struct PageDirectoryEntry){
        .flag.present_bit = 1,
        .flag.read_write = 1,
        .flag.page_size = 1,
        .lower_address = 0,
    ...
    new_page_dir->table[0x300] = _paging_kernel_page_directory.table[0x300];
